...
class ScratchItchPlusEnv(AssistiveEnv):
    def __init__(self, robot, human):
        super(ScratchItchPlusEnv, self).__init__(robot=robot, human=human, task='scratch_itch', obs_robot_len=(23 + len(robot.controllable_joint_indices) - (len(robot.wheel_joint_indices) if robot.mobile else 0)), obs_human_len=(24 + len(human.controllable_joint_indices)))
    ...
    def _get_obs(self, agent=None):
        ...
        img, _ = self.get_camera_image_depth()
        ...
        robot_obs = np.concatenate([img.flatten(), force_torque, tool_pos_real, tool_orient_real, tool_pos_real - target_pos_real, target_pos_real, robot_joint_angles, shoulder_pos_real, elbow_pos_real, wrist_pos_real, [self.tool_force]]).ravel()
        if agent == 'robot':
            return robot_obs
        if self.human.controllable:
    ...
    def reset(self):
        ...
        elbow_pos = self.human.get_pos_orient(self.human.right_forearm)[0]
        wrist_pos = self.human.get_pos_orient(self.human.right_hand)[0]
        self.create_sphere(radius=0.1, pos=shoulder_pos, collision=False)
        self.create_sphere(radius=0.1, pos=elbow_pos, collision=False)
        self.create_sphere(radius=0.1, pos=wrist_pos, collision=False)
