...
class BedBathingPlusEnv(AssistiveEnv):
    def __init__(self, robot, human):
        super(BedBathingPlusEnv, self).__init__(robot=robot, human=human, task='bed_bathing', obs_robot_len=(17 + len(robot.controllable_joint_indices) - (len(robot.wheel_joint_indices) if robot.mobile else 0)), obs_human_len=(18 + len(human.controllable_joint_indices)))
    ...
    def _get_obs(self, agent=None):
        ...
        robot_obs = np.concatenate([img.flatten(), force_torque, end_effector_pos_real, end_effector_orient_real, robot_joint_angles, shoulder_pos_real, elbow_pos_real, wrist_pos_real, [self.tool_force]]).ravel()
        if agent == 'robot':
            return robot_obs
        if self.human.controllable:
